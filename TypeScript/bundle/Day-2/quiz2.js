"use strict";
{
    ...
    class Income extends VechilePurchase {
        constructor(vehicleData) {
            super(vehicleData);
        }
        getTotalIncomeVechile(type) {
            let result = this.vehicleData.filter((vehicle) => vehicle.vehicleType == type).reduce((previous, current) => previous + current.total, 0);
            let result2 = this.vehicleData.reduce((previous, current) => previous + current.total, 0);
            if (type) {
    ...
    let object = new Income(vehicleData);
