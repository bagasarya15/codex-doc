...
// console.log(hitungLuasLingkaran(5)) // return 78.53975
...
// console.log(convertSuhu('F')) //farenhait must an number
...
const hitungDiscount = (price, tax, discount) => {
    ...
    try {
        ...
        let subTotalDiskon = price * diskon;
        let priceAfterDiscount = price - subTotalDiskon;
        let pajak = tax / 100;
        let subTotalPajak = priceAfterDiscount * pajak;
        let total = priceAfterDiscount + subTotalPajak;
        
        //kondisi jika price, tax, discount is not number
        if(isNaN(price) && isNaN(tax) && isNaN(discount))
            throw new Error('Price, Tax & Discount harus dalam angka')
        //kondisi jika price is not number
        if(isNaN(price))
            throw new Error('Price harus dalam angka')
        //kondisi jika price, tax is not number
        if(isNaN(price) || isNaN(tax))
        throw new Error('Pajak harus dalam angka')
...
const getCordinat = (x1, y1, x2, y2) => {
    //try cacth untuk menangani jika ada kesalahan pada code
    try {
        //kondisi jika x1, x2, y1, y2 is not number
...
const sumDigit = (n) => {
    ...
    try {
        ...
        //kondisi jika n lebih besar atau sama dengan 10000
            ...
            let sum = string.reduce((a, b) => Number(a) + Number (b))
...
const elapsedTime = (seconds) => {
    ...
    try {
        //if untuk jika kondisi detik bukan angka atau is not number
        if(isNaN(seconds)){
            throw new Error(`${seconds} is not a number try again...`);
        }

        //Hitung menggunakan Math.floor untuk membulatkan nilai ke bawah
        let hari  = Math.floor(seconds / (3600 * 24)); //untuk hitung hari
        let jam   = Math.floor((seconds - (hari * 3600 * 24)) / 3600); //untuk hitung jam
        let menit = Math.floor((seconds - (hari * 3600 * 24) - (jam * 3600)) / 60); // untuk hitung menit
        let detik = seconds - (hari * 3600 * 24) - (jam * 3600) - (menit * 60); // untuk hitung detik
...
const getRandomPass = (password) => {
    //try cacth untuk menangani jika ada kesalahan pada code
    try {
        //kondisi jika password bukan string
        if(typeof password !== 'string' || parseInt(password)){
            throw new Error(`${password} Password must be a character...`);
        //kondisi jika password lebih kecil dari 6
...
const convertToRupiah = (money, konversi) => {
...
// console.log(getHeavier(12,70,45)); //70
...
const getDay = (m,y) => {
    //try cacth untuk menangani jika ada kesalahan pada code
    try {
        //kondisi jika m dan y is not number
        if(isNaN(m) && isNaN(y)){
            throw new Error('inputan bulan dan tahun harus dalam integer')
        //kondisi jika m is not number
        }else if(isNaN(m)){
            throw new Error('inputan bulan harus dalam integer')
        //kondisi jika y is not number
        }else if(isNaN(y)){
            throw new Error('inputan tahun harus dalam integer')
        }
...
const getProsentase = (income1, income2) => {
    ...
    try {    
        // kondisi jika income1 dan income2 tidak berupa number
        if (typeof income1 !== "number" || typeof income2 !== "number") {
        throw new Error(`${income1} dan ${income2} harus dalam angka`);
        }
        ...
        // kondisi jika total income turun
            ...
            return `Total penurunan income ${percentage.toFixed(0)}%`; //
        } else {
            return ``;
        }
    } catch (error) {
...
const totalGaji = (s1, s2, s3) => {
    //try cacth untuk menangani jika ada kesalahan pada code
    try {
        //kondisi jika nilai params s1,s2,s3 tidak berupa number
        if (typeof s1 !== "number" || typeof s2 !== "number" || typeof s3 !== "number")
            throw new Error("Gaji harus dalam angka");
